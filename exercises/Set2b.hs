...
oddFactorial n = case n of
    1 -> 1
    n -> if even n
         then 1 * oddFactorial (n - 1)
...
leftpad s l = if (length s) /= l then leftpad (" " ++ s) l else s
...
countdown num = countdown' num "Ready!"

countdown' :: Integer -> String -> String
countdown' num str = case num of
    0 -> str ++ " Liftoff!"
    n -> countdown' (num -1 ) (str ++ " " ++ show n ++ "...")
...
smallestDivisor' num div = if mod num div == 0
                           then div
                           else smallestDivisor' num (div + 1)
...
biggestPrimeAtMost num = case num of
    0 -> 1
    1 -> 1
    n -> biggestPrimeAtMost' n 2 2

biggestPrimeAtMost' :: Integer -> Integer -> Integer -> Integer
biggestPrimeAtMost' num target fnd =
    if num == target then found
    else biggestPrimeAtMost' num (target + 1) found
    where found = if isPrime target then target else fnd
